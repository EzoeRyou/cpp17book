...
$n < n$のとき、$size \geq n$である未確定のsize個の要素数をもつ入力から、n個の標本をランダムに選択する。標本の候補はn個まで保管される。$1 \geq j \geq n$のときI[j]は保管された標本を指す。

1. 入力から最初のn個を標本として選択し、保管する。$1 < j \geq n$の範囲でI[j]にj番目の標本を保管する。tの値をnとする。I[1], ..., I[n]は現在の標本を指す。tは現在処理した入力の個数を指す。
